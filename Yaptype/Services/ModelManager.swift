...
final class ModelManager: ObservableObject {
    ...
    func isMLXInstalled(_ spec: MLXModelSpec) -> Bool {
        ...
        for case let url as URL in enumerator {
            if url.lastPathComponent == "config.json", url.path.contains(spec.huggingFaceID) {
                return true
            }
        }
        return fm.fileExists(atPath: mlxFolder(for: spec).appendingPathComponent("config.json").path)
    }
}
